...
const DECISION_OPTIONS = [
  { value: 'iterate' as const, icon: '↺', label: 'Iterar', description: 'Refinando la propuesta' },
  { value: 'scale'   as const, icon: '↑', label: 'Escalar', description: 'Llevando la idea adelante' },
  { value: 'abandon' as const, icon: '✗', label: 'Abandonar', description: 'Desarrollo detenido' },
]
...
export function DecisionDialog({ projectId, isOpen, onClose, onSuccess }: DecisionDialogProps) {
  ...
  function handleOpenChange(open: boolean) {
    if (!open) { setSelectedDecision(null); setError(null); onClose() }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  backgroundColor: isSelected ? 'var(--color-background)' : 'var(--color-surface)',
              ...
              >
                <span style={{ fontSize: 'var(--text-lg)', color: isSelected ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', lineHeight: 'var(--leading-lg)', flexShrink: 0 }} aria-hidden="true">
                  {option.icon}
                </span>
        ...
        {error && <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--color-weak-text)', marginTop: 'var(--space-2)' }}>{error}</p>}

        <p style={{ margin: 0, fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 'var(--space-2)' }}>
          Esta accion es irreversible
        </p>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-2)', marginTop: 'var(--space-4)' }}>
          <Button type="button" variant="outline" onClick={onClose} disabled={isLoading}>Cancelar</Button>
          <Button type="button" onClick={handleConfirm} disabled={!selectedDecision || isLoading}>
