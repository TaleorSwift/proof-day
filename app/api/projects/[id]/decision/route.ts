...
export async function POST(
  ...
  const { data: existing } = await supabase
    .from('projects').select('id, builder_id, decision').eq('id', id).single()
  ...
  if (existing.decision !== null)
    return NextResponse.json({ error: 'Ya existe una decisión registrada', code: 'DECISION_ALREADY_REGISTERED' }, { status: 409 })

  const { data: project, error } = await supabase
    .from('projects').update({ decision: result.data.decision, decided_at: new Date().toISOString() })
    .eq('id', id).select().single()

  if (error || !project)
    return NextResponse.json({ error: 'Error al registrar la decisión', code: 'DECISION_UPDATE_ERROR' }, { status: 500 })
