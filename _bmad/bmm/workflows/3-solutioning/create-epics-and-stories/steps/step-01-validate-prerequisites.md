...
🛑 **STOP. Before discovering documents or extracting requirements, you MUST determine the state of existing epic/story artifacts.**

Scan for existing artifacts and emit:
...
- Architecture: [PRESENT | ABSENT]
  Location: <path or "N/A">
- UX design: [PRESENT | ABSENT]
  Location: <path or "N/A">
- Epics document: [PRESENT | PARTIAL | ABSENT]
  Location: <path or "N/A">
  Coverage estimate: <percentage of PRD requirements mapped to epics/stories, or "N/A">
- Observations: <any relevant notes>
```

Then declare the execution mode:
...
- **GENERATE** (no epics exist or coverage < 30%): Execute all steps as designed — full creation.
- **REFINE** (epics exist, 30-90% coverage or gaps detected): Preserve valid epics/stories. Only add missing ones or fix gaps.
- **VERIFY** (epics exist, >= 90% coverage, aligned with PRD): Validate alignment. Do not produce new epics.
